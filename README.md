...
面向 DL 系列 AI 硬件设备的微信小程序，提供设备添加、蓝牙配网、设备管理与模型配置等能力。
...
2. **蓝牙配网**：搜索 DL 设备 → 蓝牙连接 → 选择 Wi-Fi → 提交配网 → 自动绑定
...
`utils/device-id.js` 会将设备 ID 统一规范为 `DL-` 前缀格式，例如：

- 输入 `00011C00D5AD` → `DL-00011C00D5AD`
- 输入 `DL-00011C00D5AD` → 保持不变
- 输入 `MYF-00011C00D5AD` → 自动转换为 `DL-00011C00D5AD`
